...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        ...
        case 2:{
            ...
            UIButtonLarge* sButton = [UIButtonLarge buttonWithType:UIButtonTypeCustom];
            [sButton setTitle:@"Pay online" forState:UIControlStateNormal];
            [sButton addTarget:self action:@selector(payOnline:) forControlEvents:UIControlEventTouchDown];
            sButton.frame = CGRectMake(170, 5, 100, 20);
            [cell.contentView addSubview:sButton];
            
            if ([row_order needPayMoney])
            {
                sButton.hidden = NO;
            }
            else
...
- (void) payOnline:(id)aButton
{
    UITableViewCell* sClickedCell = (UITableViewCell *)[[aButton superview] superview];
    NSIndexPath* sIndexPath = [self.tableView indexPathForCell:sClickedCell];
    
    OrderBean *row_order  =[order_list objectAtIndex:sIndexPath.section];
    
    NSString* sProductString = [NSString string];
    for (ProductBean* sProduct in row_order.product_list)
    {
        sProductString = [sProductString stringByAppendingFormat:@"%@(%@%d)", sProduct.productName, NSLocalizedString(@"Quantity", nil), sProduct.shopcount];
    }
    
    [[MyAlipay shared] startPayWithOrderNo:row_order.orderId productName:@"柚子舍产品" productDesp:sProductString amount:row_order.toPrice];
        ...
        if ([order_result_list count]<=0) {
            ...
            for (int i =0; i<[order_result_list count]; i++) {
                ...
                for (NSDictionary *product_request_dict in [order_dict objectForKey:@"orderDetails"] ) {
                    ...
                    //销售
                    [product setShopcount:[[product_request_dict objectForKey:@"quantity"] integerValue]];
