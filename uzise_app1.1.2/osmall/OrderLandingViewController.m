...
#define TAG_ALERTVIEW_FAILURE       100
#define TAG_ALERTVIEW_SUCCESS_COD   101
#define TAG_ALERTVIEW_SUCCESS_ALIPAY 102
#define TAG_ALERTVIEW_OPEN_APPSTORE_FOR_ALIPAY 105
...
@property(nonatomic,copy)NSString *mProductsString;
...
@synthesize user;
@synthesize alert,mProductsString;
...
-(void)loadLocalData
{
    ...
    payModeDict  = [NSDictionary dictionaryWithContentsOfFile:  [IOSFactory getMainPath:@"paymode" oftype:@"plist"]]  ;
    
    [shopping_dict objectForKey:@"payMode"];
    
    sendTimeDict =[NSDictionary dictionaryWithContentsOfFile:  [IOSFactory getMainPath:@"sendtime" oftype:@"plist"]]  ;
    
    [shopping_dict objectForKey:@"sendTime"];
    ...
    {
        if ([[UserManager shared] getLastPaymodeID].length > 0)
        {
            [shopping_dict setValue:[[UserManager shared] getLastPaymodeID] forKey:@"payMode"];
        }
        else
        {
...
-(void)createOrder
{
    ...
    self.mProductsString = [NSString string];
    for (ProductBean *p in sProductList)
    {
        ...
        self.mProductsString = [self.mProductsString stringByAppendingFormat:@"%@(%@%d)", p.productName, NSLocalizedString(@"Quantity", nil), p.shopcount];
    }
    ...
    if (![order_post error]) {
        ...
        if ([dict objectForKey:@"status"])
        {
            ...
            NSInteger sTag;
            if (((NSString*)[shopping_dict objectForKey:@"payMode"]).integerValue == 2)
            {
                sNotice = NSLocalizedString(@"We will ship soon", nil);
                sTag = TAG_ALERTVIEW_SUCCESS_COD;
            }
            else if (((NSString*)[shopping_dict objectForKey:@"payMode"]).integerValue == 5)
            {
                sNotice = NSLocalizedString(@"Please pay via alipay", nil);
                sTag = TAG_ALERTVIEW_SUCCESS_ALIPAY;
            }
            
            alert =[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Order created successfully", nil) message:sNotice delegate:self cancelButtonTitle:NSLocalizedString(@"Ok", nil) otherButtonTitles: nil];
            alert.tag = sTag;
        ...
        else
        {
            ...
            alert.tag = TAG_ALERTVIEW_FAILURE;
            [alert setDelegate:self];
            [alert show];
...
{    
    if (alertView.tag == TAG_ALERTVIEW_FAILURE)
    {
        if (buttonIndex==0)
        {
    ...
    else if (alertView.tag == TAG_ALERTVIEW_SUCCESS_COD)
    {
        if (buttonIndex==0)
        {
            [self dismissModalViewControllerAnimated:YES];
    ...
    else if (alertView.tag == TAG_ALERTVIEW_SUCCESS_ALIPAY)
    {
        [[MyAlipay shared] startPayWithOrderNo:@"125253232" productName:@"柚子舍商品" productDesp:self.mProductsString amount:self.mTotalprice];
        [self dimissOrderViewController];
    }
    else
    {
...
- (void)didCloseModalPanel:(PayModeViewController *)modalPanel
{
   ...
   if (key.length > 0)
   {
       ...
       [[UserManager shared] setLastPaymodeID:key];
   }
